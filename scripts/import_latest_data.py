...
def import_to_forex_rates(data: List[Dict]):
    ...
    for item in data:
        try:
            ...
            # ユニーク制約違反の場合はスキップ
            logger.debug(f"Skipping duplicate record at {item['timestamp']}: {str(e)[:100]}")
            db.rollback()
            skipped_count += 1
            continue
...
def main():
    ...
    # インポートするファイル（最新データ）
    import_files = [
        data_dir / "1minute" / "1minute.csv",
        data_dir / "15minute" / "15minute.csv", 
        data_dir / "hour" / "1hour.csv"
    ]
