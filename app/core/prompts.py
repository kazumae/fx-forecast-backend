...
- エントリー価格の設定ルール：
  - ロングの場合：現在価格以下でエントリー（押し目買い）または現在価格で即エントリー
  - ショートの場合：現在価格以上でエントリー（戻り売り）または現在価格で即エントリー
  - 逆張りは避け、トレンドに沿ったエントリーを推奨
- 予測の有効期限の設定ルール：
  - 1分足・5分足ベース：2-4時間以内
  - 15分足ベース：4-8時間以内
  - 1時間足ベース：本日中（12-24時間）
...
def get_full_prompt(logic_content: str) -> str:
    """Get full system prompt including logic content"""
